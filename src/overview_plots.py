...
import stuffr
import iono_config
from iono_config import get_config


def overview_plots(t0, t1, max_r=500, gc=20):
#    fl=glob.glob("results/*/ionogram*.h5")
    fl = glob.glob("results/**/ionogram*.h5", recursive=True)
    fl.sort()
    ...
    range_shift = config.range_shift
    #plt.pcolormesh(dtt, rvec-iono_config.range_shift*1.5, n.transpose(dBOR), vmin=0, vmax=10)
    plt.pcolormesh(dtt, rvec-range_shift*1.5, n.transpose(dBOR), vmin=0, vmax=10)
    plt.colorbar()
    plt.xlabel("Time (UTC)")
