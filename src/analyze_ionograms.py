...
def analyze_latest_sweep(ic, data_path="/dev/shm"):
    ...
    n_t=int(ic.s.freq_dur*ic.sample_rate/(ic.code_len*ic.dec))

    all_spec=n.zeros([ic.s.n_freqs, n_t, n_rg], dtype=n.float32)

    # IPP length
    dt=ic.dec*ic.code_len/float(ic.sample_rate)
    ...
    z_all=n.zeros([ic.s.n_freqs, int(ic.s.freq_dur*ic.sample_rate/ic.dec)], dtype=n.complex64)
...
#            pif=int(iono_freqs[i]/0.1)
    ...
    plt.pcolormesh(n.concatenate((iono_p_freq, [fmax+0.1])),
                   p_rvec-ic.range_shift*1.5, dB, vmin=0, vmax=ic.max_plot_dB)
    plt.title("%s %s UT\nnoise_floor=%1.2f (dB) peak SNR=%1.2f"
